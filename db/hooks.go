...
func RegisterTimestampHooks(db *gorm.DB) {
	db.Callback().Create().Before("gorm:create").Register("synapse:timestamps:create", func(db *gorm.DB) {
		now := time.Now()
	...
	db.Callback().Update().Before("gorm:update").Register("synapse:timestamps:update", func(db *gorm.DB) {
		if db.Statement.Schema != nil {
			if field := db.Statement.Schema.LookUpField("UpdatedAt"); field != nil {
